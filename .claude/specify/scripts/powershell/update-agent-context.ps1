...
$TEMPLATE_FILE = Join-Path $REPO_ROOT '.claude/specify/templates/agent-file-template.md'
...
function Validate-Environment {
    ...
    if (-not (Test-Path $TEMPLATE_FILE)) {
        ...
        Write-Info 'Ensure .claude/specify/templates/agent-file-template.md exists.'
        exit 1
    }
}
...
function New-AgentFile {
    ...
    if ($NEW_LANG -and $NEW_FRAMEWORK) {
        $techStackForTemplate = "- $NEW_LANG + $NEW_FRAMEWORK ($CURRENT_BRANCH)"
    } elseif ($NEW_LANG) {
        $techStackForTemplate = "- $NEW_LANG ($CURRENT_BRANCH)"
    } elseif ($NEW_FRAMEWORK) {
        $techStackForTemplate = "- $NEW_FRAMEWORK ($CURRENT_BRANCH)"
    }
...
function Update-SpecificAgent {
    ...
    switch ($Type) {
        ...
        'generic'  { Write-Info 'Generic agent: no predefined context file.' }
