...
describe('artifact-writer', () => {
  ...
  describe('editTaskCheckbox', () => {
    it('checks an unchecked task', () => {
      ...
      editTaskCheckbox(filePath, 'T001', true);
    ...
    it('unchecks a checked task', () => {
      ...
      editTaskCheckbox(filePath, 'T001', false);

      const result = fs.readFileSync(filePath, 'utf-8');
      expect(result).toContain('- [ ] T001 Done');
    });

    it('preserves all other content byte-for-byte', () => {
      const filePath = path.join(TEST_DIR, 'tasks.md');
  ...
  describe('editRequirementText', () => {
    it('updates requirement text preserving ID and formatting', () => {
      ...
      editRequirementText(filePath, 'FR-001', 'System MUST do new thing');
